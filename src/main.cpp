...
std::vector<Movie *> selectionSort(int loops, std::map<std::string, Movie *> &moviesByName) //Cut down to only correct number of values
{
    ...
    if (loops > moviesByName.size() or loops < 0)
        loops = moviesByName.size();
...
std::vector<Movie *> suggestAlt(std::string title, std::map<std::string, Movie *> &movies)
{
...
Movie *findMovie(std::string title, std::string year, std::map<std::string, Movie *> &movies) //Todo account for a null search result
{
    ...
    std::cout << "Movie: " << title << " not found!" << std::endl
              << std::endl;
    std::vector<Movie *> suggestions = suggestAlt(title, movies);
    sort(suggestions.begin(), suggestions.end(), [](Movie *a, Movie *b) { return a->ratings > b->ratings; });
...
void scoreMovies(std::map<std::string, Movie *> movies, Movie *selectedMovie)
{
    ...
    if (selectedMovie->year == "UNKNOWN")
        selectedMovie->year = "1900";
    ...
    for (auto it = movies.begin(); it != movies.end(); it++)
    {
        ...
        for (auto genre : thisMovie->genres)
            genreScore += selectedMovie->genres.count(genre);

        for (auto actorId : thisMovie->actorIds)
            actorScore += selectedMovie->actorIds.count(actorId);

        for (auto writerId : thisMovie->writerIds)
            writerScore += selectedMovie->writerIds.count(writerId);

        for (auto directorId : thisMovie->directorIds)
            directorScore += selectedMovie->directorIds.count(directorId);
